...
#define COL pw->plan->ncol
#define ROW pw->plan->nrow
#define PLANET pw->plan->w
#define BIRTH pw->plan->btime
#define DEATH pw->plan->dtime
#define UPDATE pw->plan->updated
...
/** tipo delle celle del pianeta:
 ...
 WATER solo acqua
 */
typedef enum cell { WATER, SHARK, FISH } cell_t;
...
/** crea una matrice vuota (tutte le celle contengono 'W')
...
/** crea una matrice vuota (tutte le celle contengono 0) utilizzando la rappresentazione con un vettore di puntatori a righe
 \param nrow numero di righe
 \param ncol numero di colonne
 \retval NULL se si sono verificati errori
 \retval m puntatore al vettore di puntatori a righe
...
/** inizializza il pianeta leggendo da file la configurazione iniziale
 ...
 ASSUNZIONE: la funzione controlla solamente se i parametri in imput garantiscono il corretto funzionamento del programma, solo nel caso in cui si verifica un errore di formattazione tale da non consentire la corretta esecuzione del programma, la simulazione termina settando errno
...
#define ALIVE 3
#define DEAD 4
/** Regola 1: gli squali mangiano e si muovono
 ...
 si muove nella cella precedentemente occupata dal pesce. Se nessuna
 delle celle adiacenti contiene un pesce, lo squalo si muove
 ...
 non possiamo ne mangiare ne muoverci lo squalo rimane fermo.
...
/** Regola 3: i pesci si muoveno
 ...
 un pesce si muove casualmente in una delle celle adiacenti (libera).
 ...
 \retval STOP se siamo rimasti fermi
 \retval MOVE se il pesce si e' muoveto
 \retval -1 se si e' verificato un errore (setta errno)
 */
